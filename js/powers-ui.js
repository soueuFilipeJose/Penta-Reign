...
(() => {
  ...
  window.addEventListener('beforeprint',()=>{
    ...
    printTitle=$('viewTitle').textContent;$('viewTitle').textContent='Penta-Reign · Ficha de personagem';
    document.querySelectorAll('#ficha .view').forEach(el=>{printPanels.push({el,hidden:el.hidden});el.hidden=false;});
  ...
  window.addEventListener('afterprint',()=>{printState.splice(0).forEach(({el,open})=>el.open=open);printPanels.splice(0).forEach(({el,hidden})=>el.hidden=hidden);$('viewTitle').textContent=printTitle;document.querySelectorAll('#ficha textarea').forEach(el=>el.style.height=el.dataset.printHeight||'');const identity=$('identityBody');if(identity)identity.hidden=identity.dataset.printHidden==='true';});
  renderWorkbench();setupFiles();renderPowerGifts(count(),true);refreshPowerOutputs();
})();
